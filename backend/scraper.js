...
async function checkSemanticDuplicate(newTitle, newSummary, candidateArticles) {
    ...
    try {
        const response = await retryWithBackoff(() => ai.models.generateContent({
            model: 'gemini-flash-lite-latest',
            contents: `Αξιολόγησε αν το ΝΕΟ ΑΡΘΡΟ αναφέρεται ΣΤΗΝ ΙΔΙΑ ΕΙΔΗΣΗ / ΘΕΜΑ / ΜΕΤΑΓΡΑΦΙΚΗ ΕΞΕΛΙΞΗ με κάποιο από τα ΠΙΘΑΝΑ ΥΠΑΡΧΟΝΤΑ ΑΡΘΡΑ.

ΚΑΝΟΝΕΣ:
1. Αν πρόκειται για μεταγραφικό ρεπορτάζ, ενδιαφέρον ή εξέλιξη deal για το ΙΔΙΟ πρόσωπο (π.χ. Λιβάι Γκαρσία στον Παναθηναϊκό) από διαφορετικά μέσα, ΘΕΩΡΕΙΤΑΙ ΤΟ ΙΔΙΟ ΘΕΜΑ και πρέπει να γίνει merge.
2. Διαφορετικά θέματα (π.χ. διαφορετικοί παίκτες, ρεπορτάζ αγώνα vs μεταγραφή, δηλώσεις άλλου προσώπου) ΔΕΝ συγχωνεύονται.
