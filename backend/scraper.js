    ...
    try {
        ...
        if (extractedDateStr) {
            ...
            if (!isNaN(pubDate.getTime())) {
                ...
                // MAX AGE CHECK: Skip articles published more than 24 hours ago!
                if (ageHours > 24) {
                    console.log(`  [PARSING WARNING] Article is too old (Published: ${pubDate.toISOString()}, Age: ${ageHours.toFixed(1)}h). Maximum allowed age is 24h. Skipping ${url}`);
                    return { status: 'skipped_older', length: 0 };
                }
            }
        }
...
async function checkSemanticDuplicate(newTitle, newSummary, candidateArticles) {
    ...
    try {
        const response = await retryWithBackoff(() => ai.models.generateContent({
            model: 'gemini-flash-lite-latest',
            contents: `Αξιολόγησε αν το ΝΕΟ ΑΡΘΡΟ και κάποιο από τα ΥΠΑΡΧΟΝΤΑ ΑΡΘΡΑ καλύπτουν το **ίδιο συγκεκριμένο γεγονός** (ίδια χρονική στιγμή, ίδιο θέμα).

ΚΑΝΟΝΕΣ:
1. ΣΥΓΧΩΝΕΥΕ αν αναφέρονται στο **ίδιο γεγονός ή στιγμή**, έστω κι αν το καλύπτουν από διαφορετική γωνία:
   - «Ο Γιαμπουσελέ σε εξαιρετική φυσική κατάσταση» + «Εντυπωσιακός ο Γιαμπουσελέ στη Μύκονο» → MERGE (ίδια εμφάνιση).
   - «Επίσημο: Στον ΠΑΟ ο Γκαρσία» + «Ανακοίνωσε τον Γκαρσία ο ΠΑΟ» → MERGE.
   - «Τι είπε ο Αταμάν στη συνέντευξη» + «Οι δηλώσεις Αταμάν μετά τον αγώνα» → MERGE.
