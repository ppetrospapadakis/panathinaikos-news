...
async function checkSemanticDuplicate(newTitle, newSummary, candidateArticles) {
    ...
    try {
        const response = await retryWithBackoff(() => ai.models.generateContent({
            model: 'gemini-flash-lite-latest',
            contents: `Αξιολόγησε αν το ΝΕΟ ΑΡΘΡΟ αναφέρεται ΣΤΗΝ ΙΔΙΑ ΕΙΔΗΣΗ / ΘΕΜΑ / ΑΓΩΝΑ / ΜΕΤΑΓΡΑΦΗ με κάποιο από τα ΠΙΘΑΝΑ ΥΠΑΡΧΟΝΤΑ ΑΡΘΡΑ.

ΚΑΝΟΝΕΣ:
1. ΜΕΤΑΓΡΑΦΕΣ: Αν πρόκειται για μεταγραφικό ρεπορτάζ, ενδιαφέρον ή εξέλιξη deal για το ΙΔΙΟ πρόσωπο (π.χ. Λιβάι Γκαρσία στον Παναθηναϊκό) από διαφορετικά ή ίδια μέσα, ΘΕΩΡΕΙΤΑΙ ΤΟ ΙΔΙΟ ΘΕΜΑ και πρέπει να γίνει merge.
2. ΑΓΩΝΕΣ / ΠΡΟΕΤΟΙΜΑΣΙΑ / ΠΡΟΑΝΑΓΓΕΛΙΑ: Αν πρόκειται για τον ΙΔΙΟ αγώνα ή την προετοιμασία/προαναγγελία/ευρωπαϊκή πρεμιέρα για τον ΙΔΙΟ αγώνα (π.χ. Παναθηναϊκός - ΤΣΣΚΑ 1948, Ευρωπαϊκή πρεμιέρα με ΤΣΣΚΑ, Στο ΟΑΚΑ με ΤΣΣΚΑ 1948), ΘΕΩΡΕΙΤΑΙ ΤΟ ΙΔΙΟ ΘΕΜΑ και πρέπει να γίνει merge.
3. ΔΗΛΩΣΕΙΣ / ΣΥΝΕΝΤΕΥΞΕΙΣ: Αν πρόκειται για τις ίδιες δηλώσεις ή συνέντευξη τύπου του ΙΔΙΟΥ προσώπου, ΘΕΩΡΕΙΤΑΙ ΤΟ ΙΔΙΟ ΘΕΜΑ και πρέπει να γίνει merge.
4. Μόνο αν πρόκειται για εντελώς διαφορετικά θέματα (π.χ. διαφορετικοί παίκτες, διαφορετικοί αγώνες) ΔΕΝ συγχωνεύονται.

Αν ταυτίζεται με κάποιο άρθρο, επίστρεψε ΑΠΟΚΛΕΙΣΤΙΚΑ το ID του ταυτιζόμενου άρθρου. Αν όχι, επίστρεψε "null". ΜΗΝ δικαιολογήσεις την απάντησή σου.
