...
// 1. Put comment badge placeholder next to date / official badge
html = html.replace(
...
// 2. Inject lazy loaded comments placeholder div - target exactly the closing </article> tag
...
html = html.replace(displayTarget, displayReplacement);
...
// 4. Inject Comments script block before original showError function
...
html = html.replace(originalShowError, commentsLogic);
...
html = html.replace(initAnchor, initAnchorPatch);

fs.writeFileSync('article.html', html, 'utf8');
console.log('Final clean patch written.');
