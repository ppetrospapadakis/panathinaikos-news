...
// 1. Add slugify & getCategoryCleanName helpers if missing in roster.html
const helpersCode = `
...
if (!content.includes('function getCategoryCleanName')) {
    content = content.replace('// ─────────────────────────────────────────────────────────────────\n        let streamPage = 1;', `${helpersCode}\n        // ─────────────────────────────────────────────────────────────────\n        let streamPage = 1;`);
}

// 2. Fix fetchMoreRosterNews in roster.html
const oldFetchMoreRosterNewsRegex = /async function fetchMoreRosterNews\(\) \{[\s\S]*?^\s*\}/m;
const newFetchMoreRosterNews = `async function fetchMoreRosterNews() {
...
content = content.replace(oldFetchMoreRosterNewsRegex, newFetchMoreRosterNews);

fs.writeFileSync(rosterPath, content, 'utf8');
console.log('✅ Updated roster.html fetchMoreRosterNews infinite scroll & SEO article links.');
